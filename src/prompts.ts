...
export function formatItem(item: GitHubItem, lang: "zh" | "en" | "vi" = "zh"): string {
...
function sampleNote(total: number, sampled: number, lang: "zh" | "en" | "vi" = "zh"): string {
  if (lang === "vi") {
...
export function buildCliPrompt(
  ...
  const issuesText =
    ...
    (lang === "vi" ? "Không có" : lang === "en" ? "None" : "无");
  const prsText =
...
Hãy tạo một bản tin có cấu trúc rõ ràng bằng tiếng Việt, bao gồm các phần sau:

1. **Điểm nổi bật hôm nay** - 2-3 câu tóm tắt các cập nhật quan trọng nhất
2. **Phát hành phiên bản** - Nếu có phiên bản mới, tóm tắt nội dung cập nhật; bỏ qua nếu không có
3. **Issues nóng trong cộng đồng** - Chọn 10 Issue đáng chú ý nhất, giải thích tại sao quan trọng và phản ứng của cộng đồng
4. **Tiến độ PR quan trọng** - Chọn 10 PR quan trọng, mô tả tính năng hoặc bản sửa lỗi
5. **Xu hướng yêu cầu tính năng** - Trích xuất các hướng tính năng được cộng đồng quan tâm nhất từ tất cả Issues
6. **Điểm đau của nhà phát triển** - Tóm tắt các vấn đề thường gặp hoặc yêu cầu có tần suất cao từ phản hồi của nhà phát triển

Yêu cầu ngôn ngữ: tiếng Việt, chuyên nghiệp và ngắn gọn, phù hợp cho nhà phát triển kỹ thuật. Mỗi mục đính kèm liên kết GitHub.
`;
  }
...
export function buildPeerPrompt(
  ...
  const noneStr = lang === "vi" ? "Không có" : lang === "en" ? "None" : "无";
...
- PRs cập nhật trong 24 giờ qua: ${totalPrs} (mở: ${openPrs}, đã merge/đóng: ${mergedPrs})
- Phiên bản mới: ${releases.length}
...
Hãy tạo bản tin dự án ${cfg.name} có cấu trúc rõ ràng bằng tiếng Việt, bao gồm các phần sau:

1. **Tổng quan hôm nay** - 3-5 câu tóm tắt trạng thái dự án, bao gồm đánh giá mức độ hoạt động
2. **Phát hành phiên bản** - Nếu có phiên bản mới, chi tiết nội dung thay đổi, thay đổi phá vỡ, lưu ý di chuyển; bỏ qua nếu không có
3. **Tiến độ dự án** - PRs đã merge/đóng hôm nay, tính năng nào được đẩy mạnh hoặc sửa lỗi
4. **Chủ đề nóng trong cộng đồng** - Issues/PRs hoạt động nhất với nhiều bình luận/phản ứng nhất (đính kèm liên kết), phân tích nhu cầu cơ bản
5. **Lỗi & Ổn định** - Lỗi, sự cố, hồi quy được báo cáo hôm nay, xếp hạng theo mức độ nghiêm trọng, ghi chú nếu đã có PR sửa lỗi
6. **Yêu cầu tính năng & tín hiệu lộ trình** - Tính năng do người dùng yêu cầu, dự đoán tính năng nào có thể được đưa vào phiên bản tiếp theo
7. **Tóm tắt phản hồi người dùng** - Điểm đau thực tế của người dùng, trường hợp sử dụng, mức độ hài lòng/không hài lòng
8. **Theo dõi tồn đọng** - Issues hoặc PRs quan trọng lâu chưa được phản hồi, cần sự chú ý của người bảo trì

Yêu cầu ngôn ngữ: tiếng Việt, khách quan, dựa trên dữ liệu, nổi bật sức khỏe dự án. Mỗi mục đính kèm liên kết GitHub.
`;
  }
...
export function buildPeersComparisonPrompt(
  ...
  const openclawSection =
    lang === "vi"
      ? `## OpenClaw (tham chiếu chính, github.com/${openclawDigest.config.repo})\n${openclawDigest.summary}`
      : lang === "en"
  ...
  if (lang === "vi") {
    return `Bạn là một nhà phân tích cấp cao về hệ sinh thái mã nguồn mở AI agent và trợ lý AI cá nhân. Sau đây là tóm tắt bản tin cộng đồng ngày ${dateStr} của từng dự án.
...
Hãy tạo báo cáo so sánh chéo giữa các dự án bằng tiếng Việt với các phần sau:

1. **Tổng quan hệ sinh thái** - 3-5 câu về bức tranh tổng thể của hệ sinh thái mã nguồn mở trợ lý AI cá nhân / agent
2. **So sánh mức độ hoạt động** - Bảng so sánh số lượng Issues, PRs, trạng thái Release và điểm sức khỏe của từng dự án
3. **Vị thế của OpenClaw** - Ưu thế so với các dự án cùng loại, sự khác biệt về hướng kỹ thuật, so sánh quy mô cộng đồng
4. **Hướng kỹ thuật chung** - Yêu cầu xuất hiện ở nhiều dự án (ghi rõ dự án nào, nhu cầu cụ thể)
5. **Phân tích khác biệt hóa** - Sự khác biệt chính về trọng tâm tính năng, người dùng mục tiêu, kiến trúc kỹ thuật
6. **Động lực & độ trưởng thành của cộng đồng** - Phân tầng hoạt động, dự án nào đang lặp nhanh, dự án nào đang ổn định
7. **Tín hiệu xu hướng** - Xu hướng ngành được trích xuất từ phản hồi cộng đồng, giá trị tham khảo cho nhà phát triển AI agent

Yêu cầu ngôn ngữ: tiếng Việt, ngắn gọn và chuyên nghiệp, có dữ liệu hỗ trợ, phù hợp cho người ra quyết định kỹ thuật và nhà phát triển.
`;
  }
...
export function buildSkillsPrompt(
  ...
  if (lang === "vi") {
    return `Bạn là một nhà phân tích kỹ thuật chuyên về hệ sinh thái Claude Code. Dữ liệu sau đây từ github.com/anthropics/skills (kho Skills chính thức của Claude Code). Phân tích hoạt động Skills được cộng đồng quan tâm nhất (dữ liệu tính đến ${dateStr}).

## Giới thiệu kho lưu trữ
anthropics/skills là bộ sưu tập Skills chính thức của Claude Code. Mỗi PR thường tương ứng với một Skill mới hoặc cải tiến. Cộng đồng đề xuất Skill mới và báo cáo vấn đề qua Issues; PR đại diện cho các Skill thực tế được gửi lên.

## Pull Requests nổi bật (sắp xếp theo bình luận, tổng ${prs.length}, hiển thị ${topPrs.length} đầu)
${prsText}

## Issues cộng đồng (sắp xếp theo bình luận, tổng ${issues.length}, hiển thị ${topIssues.length} đầu)
${issuesText}

---

Hãy tạo báo cáo điểm nổi bật của cộng đồng Claude Code Skills bằng tiếng Việt với các phần sau:

1. **Xếp hạng Skills hot** - Liệt kê 5-8 Skills (PR) được thảo luận nhiều nhất, mô tả chức năng, điểm nổi bật thảo luận và trạng thái hiện tại (open/merged/draft)
2. **Xu hướng nhu cầu cộng đồng** - Từ Issues, trích xuất các hướng Skill mới được mong đợi nhất (như tự động hóa workflow, kiểm tra code, tạo test, tài liệu)
3. **Skills tiềm năng cao chưa merge** - PR có bình luận tích cực nhưng chưa được merge; các Skill này có thể sớm được triển khai
4. **Nhận định hệ sinh thái Skills** - Một câu tóm tắt: nhu cầu tập trung nhất của cộng đồng ở cấp độ Skills là gì?

Yêu cầu ngôn ngữ: tiếng Việt, ngắn gọn và chuyên nghiệp, mỗi mục đính kèm liên kết GitHub.
`;
  }
...
export function buildComparisonPrompt(
  ...
  if (lang === "vi") {
    return `Bạn là một nhà phân tích kỹ thuật cấp cao về hệ sinh thái công cụ phát triển AI. Sau đây là tóm tắt bản tin cộng đồng ngày ${dateStr} của từng công cụ AI CLI chính:

${sections}

---

Hãy tạo báo cáo so sánh chéo giữa các công cụ bằng tiếng Việt với các phần sau:

1. **Tổng quan hệ sinh thái** - 3-5 câu về bức tranh phát triển tổng thể của các công cụ AI CLI
2. **So sánh mức độ hoạt động** - Bảng so sánh số lượng Issues, PRs, trạng thái Release của từng công cụ hôm nay
3. **Hướng tính năng chung** - Yêu cầu xuất hiện ở nhiều cộng đồng công cụ (ghi rõ công cụ nào, nhu cầu cụ thể)
4. **Phân tích khác biệt hóa** - Sự khác biệt về trọng tâm tính năng, người dùng mục tiêu và hướng kỹ thuật
5. **Động lực & độ trưởng thành của cộng đồng** - Công cụ nào có cộng đồng hoạt động nhất, công cụ nào đang lặp nhanh
6. **Tín hiệu xu hướng** - Xu hướng ngành từ phản hồi cộng đồng, giá trị tham khảo cho nhà phát triển
...
export function buildTrendingPrompt(
  ...
  const trendingSection =
    data.trendingFetchSuccess && data.trendingRepos.length > 0
      ? data.trendingRepos
          ...
          .join("\n")
      : lang === "vi"
        ? "(Không thể lấy danh sách GitHub Trending hôm nay)"
        : lang === "en"
  ...
  if (lang === "vi") {
    return `Bạn là một nhà phân tích kỹ thuật chuyên về hệ sinh thái mã nguồn mở AI. Sau đây là dữ liệu kho lưu trữ AI hot trên GitHub ngày ${dateStr}. Hãy lọc theo liên quan AI, phân loại và phân tích xu hướng.

## Nguồn dữ liệu
- **Danh sách Trending** (github.com/trending, số sao hôm nay đáng tin cậy nhất): Danh sách hot thời gian thực với số sao mới hôm nay
- **Tìm kiếm chủ đề** (GitHub Search API, thẻ topic): Dự án liên quan AI hoạt động trong 7 ngày qua, phân nhóm theo chủ đề

---

## GitHub Trending hôm nay (${data.trendingRepos.length} kho lưu trữ)
${trendingSection}

---

## Kết quả tìm kiếm chủ đề AI (${data.searchRepos.length} kho lưu trữ, đã loại trùng)
${searchSection}

---

Hãy tạo Báo cáo Xu hướng Mã nguồn mở AI có cấu trúc bằng tiếng Việt:

**Bước 1 (Lọc)**: Từ dữ liệu trên, chọn các dự án liên quan rõ ràng đến AI/ML (loại trừ công cụ chung, framework frontend, game, v.v.). Bỏ qua các repo trending không liên quan AI.

**Bước 2 (Phân loại)**: Nhóm các dự án đã lọc vào các danh mục sau (một dự án có thể thuộc nhiều nhóm; chọn nhóm chính):
- 🔧 Cơ sở hạ tầng AI (framework, SDK, động cơ suy luận, công cụ phát triển, CLI)
- 🤖 AI Agent / Workflow (framework agent, tự động hóa, hệ thống đa agent)
- 📦 Ứng dụng AI (ứng dụng cụ thể, giải pháp ngành dọc)
- 🧠 LLM / Huấn luyện (trọng số mô hình, framework huấn luyện, công cụ fine-tuning)
- 🔍 RAG / Tri thức (cơ sở dữ liệu vector, tăng cường truy xuất, quản lý tri thức)

**Bước 3 (Xuất báo cáo)** với các phần sau:

1. **Điểm nổi bật hôm nay** — 3-5 câu về các phát triển mã nguồn mở AI đáng chú ý nhất hôm nay

2. **Dự án hot theo danh mục** — Mỗi danh mục liệt kê 3-8 dự án tiêu biểu, mỗi dự án bao gồm:
   - Tên dự án (đính kèm liên kết)
   - Dữ liệu sao (tổng + mới hôm nay, nếu có)
   - Một câu mô tả: dự án là gì và tại sao đáng chú ý hôm nay

3. **Phân tích tín hiệu xu hướng** — 200-300 từ, trích xuất từ danh sách hot hôm nay:
   - Loại công cụ AI nào đang nhận được sự chú ý bùng nổ từ cộng đồng?
   - Có ngăn xếp công nghệ hoặc hướng mới nào xuất hiện lần đầu không?
   - Liên hệ với các sự kiện phát hành LLM / ngành gần đây

4. **Điểm nóng cộng đồng** — Danh sách 3-5 dự án hoặc hướng cụ thể đáng được nhà phát triển chú ý, kèm lý do ngắn

Yêu cầu ngôn ngữ: tiếng Việt, chuyên nghiệp và ngắn gọn, phải đính kèm liên kết GitHub cho mọi dự án.
`;
  }
...
export function buildWebReportPrompt(
  ...
  const siteSections = results
    .map(({ siteName, isFirstRun, newItems, totalDiscovered }) => {
      const mode =
        lang === "vi"
          ? isFirstRun
            ? `Thu thập toàn bộ lần đầu (sitemap tổng ${totalDiscovered} URL, hiển thị ${newItems.length} bài mới nhất)`
            : `Cập nhật tăng dần, ${newItems.length} bài mới hôm nay`
          : lang === "en"
            ? isFirstRun
      ...
      const unableToExtract =
        lang === "vi"
          ? "(Không thể trích xuất nội dung văn bản)"
          : lang === "en"
  ...
  const firstRunNote =
    lang === "vi"
      ? isAnyFirstRun
        ? "Đây là lần thu thập toàn bộ đầu tiên. Hãy tập trung vào bức tranh nội dung tổng thể, bối cảnh lịch sử và chủ đề cốt lõi của mỗi trang, thay vì từng bài riêng lẻ."
        : "Đây là cập nhật tăng dần. Hãy tập trung vào nội dung mới hôm nay và đánh giá ý nghĩa chiến lược trong bối cảnh."
      : lang === "en"
        ? isAnyFirstRun
  ...
  if (lang === "vi") {
    return `Bạn là một nhà phân tích nội dung chuyên sâu về AI, giỏi trích xuất tín hiệu chiến lược từ thông báo chính thức, blog kỹ thuật, bài nghiên cứu và tài liệu sản phẩm.

Nội dung sau được thu thập ngày ${dateStr} từ Anthropic (claude.com / anthropic.com) và OpenAI (openai.com). ${firstRunNote}

${siteSections}

---

Hãy tạo Báo cáo Theo dõi Nội dung Chính thức AI chi tiết bằng tiếng Việt với các phần sau:

1. **Điểm nổi bật hôm nay** — 3-5 câu về các bản phát hành hoặc phát triển mới quan trọng nhất, nhấn mạnh điểm nổi bật chính

2. **Nội dung nổi bật Anthropic / Claude** — Sắp xếp nội dung quan trọng theo danh mục (tin tức / nghiên cứu / kỹ thuật / học, v.v.):
   - Mỗi bài dùng 2-4 câu trích xuất nhận định cốt lõi, chi tiết kỹ thuật hoặc ý nghĩa kinh doanh
   - Ghi chú ngày xuất bản và liên kết gốc
   - Nếu lần thu thập đầu tiên, theo dõi các cột mốc quan trọng theo thời gian

3. **Nội dung nổi bật OpenAI** — Cùng cấu trúc, sắp xếp theo nghiên cứu / phát hành / công ty / an toàn

4. **Phân tích tín hiệu chiến lược** — Dựa trên nhịp độ phát hành và trọng tâm nội dung của hai công ty, phân tích:
   - Ưu tiên kỹ thuật gần đây của mỗi công ty (năng lực mô hình / an toàn / sản phẩm hóa / hệ sinh thái)
   - Động thái cạnh tranh: ai đang dẫn dắt chương trình nghị sự, ai đang theo sau
   - Tác động tiềm năng đến nhà phát triển và người dùng doanh nghiệp

5. **Chi tiết đáng chú ý** — Trích xuất tín hiệu ẩn từ tiêu đề, cách diễn đạt và thời điểm, ví dụ:
   - Thuật ngữ hoặc chủ đề mới xuất hiện lần đầu
   - Phát hành dày đặc trong một danh mục (có thể báo hiệu cột mốc sản phẩm)
   - Thông tin về chính sách, tuân thủ và an toàn

${isAnyFirstRun ? "6. **Tổng quan bức tranh nội dung** — Chỉ dành cho lần thu thập đầu tiên: tổng hợp phân bố danh mục nội dung của hai công ty và mô tả phong cách chiến lược nội dung (học thuật vs sản phẩm vs câu chuyện người dùng, v.v.)\n\n" : ""}Yêu cầu ngôn ngữ: tiếng Việt, chuyên nghiệp và chi tiết, phù hợp cho nhà nghiên cứu AI, quản lý sản phẩm và người ra quyết định kỹ thuật. Mỗi mục phải đính kèm liên kết chính thức.
`;
  }
...
export function buildWeeklyPrompt(
  ...
  if (lang === "vi") {
    return `Bạn là một nhà phân tích kỹ thuật chuyên về hệ sinh thái mã nguồn mở AI. Sau đây là tóm tắt bản tin hàng ngày từ 7 ngày qua (${weekStr}) về hoạt động cộng đồng công cụ AI. Hãy tạo báo cáo tổng hợp tuần.

${digestEntries}

---

Hãy tạo Báo cáo Tuần Hệ sinh thái Công cụ AI với các phần sau:

1. **Tin nổi bật tuần** - 5-8 sự kiện, bản phát hành và phát triển cộng đồng quan trọng nhất tuần này, mỗi mục kèm ngày
2. **Tiến độ công cụ CLI** - Hoạt động tổng thể và thay đổi chính của từng công cụ AI CLI (Claude Code, Codex, Gemini CLI, v.v.)
3. **Hệ sinh thái AI Agent** - Phát triển chính từ OpenClaw và các dự án cùng loại tuần này
4. **Xu hướng mã nguồn mở** - Hướng kỹ thuật đáng chú ý nhất từ GitHub Trending và cộng đồng AI tuần này
5. **Điểm nổi bật cộng đồng HN** - Chủ đề thảo luận AI chính và tâm lý cộng đồng trên Hacker News tuần này
6. **Thông báo chính thức** - Nội dung quan trọng được Anthropic và OpenAI xuất bản tuần này (nếu có)
7. **Tín hiệu tuần tới** - Dựa trên dữ liệu tuần này, dự đoán xu hướng và sự kiện sắp tới đáng theo dõi

Yêu cầu ngôn ngữ: tiếng Việt, ngắn gọn và chuyên nghiệp, giúp nhà phát triển nắm bắt nhanh diễn biến tuần.
`;
  }
...
export function buildMonthlyPrompt(
  ...
  if (lang === "vi") {
    return `Bạn là một nhà phân tích kỹ thuật chuyên về hệ sinh thái mã nguồn mở AI. Sau đây là tóm tắt bản tin cộng đồng công cụ AI tháng ${monthStr} (tổng ${Object.keys(sourceDigests).length} báo cáo). Hãy tạo báo cáo tổng hợp tháng.

${digestEntries}

---

Hãy tạo Báo cáo Tháng Hệ sinh thái Công cụ AI với các phần sau:

1. **Tin nổi bật tháng** - 5-10 sự kiện và cột mốc quan trọng nhất tháng này, theo thứ tự thời gian
2. **Tiến độ công cụ CLI tháng** - Quỹ đạo phát triển tổng thể, bản phát hành lớn và tăng trưởng cộng đồng của từng công cụ AI CLI chính
3. **Đánh giá tháng hệ sinh thái AI Agent** - Thay đổi cấu trúc hệ sinh thái, dự án mới nổi, tín hiệu đáng chú ý tháng này
4. **Tóm tắt xu hướng kỹ thuật** - Hướng kỹ thuật và chuyển đổi mô hình đáng chú ý nhất trong mã nguồn mở AI tháng này
5. **Đánh giá sức khỏe cộng đồng** - So sánh hoạt động tháng giữa các dự án chính, đánh giá mức độ tham gia của nhà phát triển
6. **Đánh giá thông báo chính thức** - Phân tích chiến lược nội dung Anthropic và OpenAI xuất bản tháng này
7. **Triển vọng tháng tới** - Dựa trên xu hướng tháng này, dự đoán hướng chính và sự kiện tiềm năng đáng theo dõi

Yêu cầu ngôn ngữ: tiếng Việt, phân tích sâu, dựa trên dữ liệu, phù hợp cho đánh giá tháng và ra quyết định chiến lược.
`;
  }
...
export function buildHnPrompt(data: HnData, dateStr: string, lang: "zh" | "en" | "vi" = "zh"): string {
    ...
    .map((s, i) =>
      lang === "vi"
        ? `${i + 1}. **${s.title}**\n` +
          `   Liên kết: ${s.url}\n` +
          `   Thảo luận: ${s.hnUrl}\n` +
          `   Điểm: ${s.points} | Bình luận: ${s.comments} | Tác giả: @${s.author} | Thời gian: ${s.createdAt.slice(0, 16)}`
        : lang === "en"
          ? `${i + 1}. **${s.title}**\n` +
            `   Link: ${s.url}\n` +
            `   Discussion: ${s.hnUrl}\n` +
  ...
  if (lang === "vi") {
    return `Bạn là nhà phân tích tin tức ngành AI. Sau đây là các bài đăng hot liên quan AI trên Hacker News trong 24 giờ qua tính đến ${dateStr} (sắp xếp theo điểm, tổng ${data.stories.length}):
...
Hãy tạo Bản tin Cộng đồng AI Hacker News có cấu trúc bằng tiếng Việt:

1. **Điểm nổi bật hôm nay** — 3-5 câu về chủ đề thảo luận AI nóng nhất và tâm lý cộng đồng trên HN hôm nay

2. **Tin tức & Thảo luận hot** — Sắp xếp theo danh mục, chọn 2-5 mục tiêu biểu nhất mỗi danh mục, mỗi mục gồm:
   - Tiêu đề (kèm liên kết gốc) + liên kết thảo luận HN
   - Điểm và số bình luận
   - Một câu giải thích: tại sao đáng chú ý, phản ứng điển hình của cộng đồng

   Danh mục:
   - 🔬 Mô hình & Nghiên cứu (phát hành mô hình mới, bài báo, benchmark)
   - 🛠️ Công cụ & Kỹ thuật (dự án mã nguồn mở, framework, thực hành kỹ thuật)
   - 🏢 Tin tức Ngành (tin công ty, gọi vốn, ra mắt sản phẩm)
   - 💬 Quan điểm & Tranh luận (bài Ask HN, Show HN đáng chú ý hoặc chủ đề tranh luận nóng)

3. **Tín hiệu Tâm lý Cộng đồng** — 100-200 từ phân tích tâm lý và trọng tâm thảo luận AI trên HN hôm nay:
   - Chủ đề nào được cộng đồng quan tâm nhất (điểm cao + bình luận nhiều)?
   - Có điểm tranh cãi hoặc đồng thuận rõ ràng nào không?
   - So với chu kỳ trước, hướng quan tâm có thay đổi đáng kể không?

4. **Đáng đọc sâu** — Liệt kê 2-3 nội dung đáng để lập trình viên/nhà nghiên cứu đọc sâu nhất hôm nay, giải thích ngắn gọn lý do

Yêu cầu ngôn ngữ: tiếng Việt, ngắn gọn và chuyên nghiệp, giữ nguyên tất cả liên kết gốc.
`;
  }
